...
| 05 | [Data Model](05-data-model.md) | The concrete schema — every relational entity as a Postgres table, plus the Qdrant vector store (one collection per Organization) that replaced the `resume_chunks` table, mapped back to which invariants they enforce. |
| 06 | [Architecture](06-architecture.md) | Components (separate Python backend, RAG vector search pipeline against a dedicated Qdrant store, multi-model LLM crew), the end-to-end and search data flows, the sync/async boundary, and the multi-tenancy isolation design across Postgres and Qdrant. |
| 07 | [Technical Stack](07-technical-stack.md) | Concrete technology choices per layer — Python/FastAPI backend, PostgreSQL for relational data, Qdrant for the vector store, CrewAI multi-agent orchestration, per-task model assignment — each with a one-line rationale and the alternative rejected. |
