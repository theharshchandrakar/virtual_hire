...
| Qdrant Cloud **[New 2026-07-15]** | Resume chunk text + embeddings, at rest, for every Organization | Hosts the vector store the RAG search feature queries against |

All three are data **subprocessors** of Sift with respect to each Organization's candidate data. **[NEEDS LEGAL REVIEW — a Data Processing Agreement (or equivalent subprocessor terms) must be in place with all three providers, and each Organization's own DPA with Sift must disclose these as subprocessors, before this pipeline processes real candidate data in a regulated jurisdiction. Qdrant Cloud is a new addition to this list and has not been through the same review the original two providers are pending — treat it as equally blocking.]**
